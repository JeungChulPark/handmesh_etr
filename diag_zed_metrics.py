...
Reads <save_dir>/metrics.jsonl (written by infer_rgbd_zed.py --save), similarity-
...
diag_pose_vs_root.py computes on the iPhone set (iPhone early-fusion median ~51%).

Run:  python diag_zed_metrics.py <save_dir>
"""
...
def main():
    ...
    if not os.path.exists(path):
        print(f"[diag] no metrics.jsonl in {d} (run infer_rgbd_zed.py --save {d})")
        return
    pose_e, rzs = [], []
