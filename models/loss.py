...
def p_mpjpe(predicted, target):
    ...
    # Return MPJPE
    return np.mean(np.linalg.norm(predicted_aligned - target, axis=len(target.shape)-1)) * 1000

def pa_mpjpe_loss(pred, gt):
    pa_mpjpe = 0.0
    num_samples = pred.size(0)
    import pdb; pdb.set_trace()
    
    for i in range(num_samples):
        est_pose = pred[i]  # Replace with your estimated pose data (a torch tensor)
        gt_pose = gt[i]          # Replace with your ground truth pose data (a torch tensor)

        # Perform Procrustes alignment using NumPy (as scipy's procrustes doesn't support torch tensors)
        est_pose_np = est_pose.cpu().numpy()
        gt_pose_np = gt_pose.cpu().numpy()
        _, r, t = procrustes(gt_pose_np, est_pose_np)

        aligned_predicted_pose = np.dot(est_pose_np - t, r)
        aligned_mpjpe = np.linalg.norm(gt_pose_np - aligned_predicted_pose, axis=-1).mean()
        # Convert the aligned pose back to a torch tensor
        
        # Calculate the per joint position error
        

        pa_mpjpe += aligned_mpjpe
    pa_mpjpe /= num_samples
