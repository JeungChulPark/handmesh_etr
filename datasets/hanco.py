...
class Hanco_with_Mediapipe(Dataset):
    def __init__(self, config, set_type="train", limit=1e5, device='cpu'):
        self.device = device
        self.image_dir = os.path.join(config.HANCO_DATA_ROOT, "rgb")
        self.img_size = config.IMG_SIZE

        mp_anno = os.path.join(config.HANCO_DATA_ROOT, "mediapipe_pixel.json")
        self.train_xyz = os.path.join(config.HANCO_DATA_ROOT, "xyz")
        self.train_cam = os.path.join(config.HANCO_DATA_ROOT, "calib")
        ...
        for scene_num, scene_dict in self.mp_anno.items():
            for cam_id, cam_dict in scene_dict.items():
                for img_name, img_dict in cam_dict.items():
                    ...
                    if os.path.isfile(os.path.join(self.train_cam, scene_num, img_id + '.json')) and os.path.isfile(os.path.join(self.train_xyz, scene_num, img_id + '.json')):
                        train_dict = {}
                        ...
                        with open(os.path.join(self.train_cam, scene_num, img_id + '.json'), "r") as f:
                            ...
                            train_dict['cam_anno'] = cam_anno['K'][cam_num]
                        ...
                        # self.image_ls.append(os.path.join(scene_num, cam_id, img_id))
                        # self.keypoints_2d.append(img_dict)
        ...
        random.shuffle(self.train_dict_list)
        


        self.start_idx = 0
        self.end_idx = len(self.train_dict_list)
        if set_type == 'train':
            self.end_idx = int(0.9 * len(self.train_dict_list))
        elif set_type == 'eval':
            self.start_idx = int(0.9 * len(self.train_dict_list))
            self.end_idx = int(0.97 * len(self.train_dict_list))
        elif set_type == 'test':
            self.start_idx = int(0.97 * len(self.train_dict_list))

        self.train_dict_list = self.train_dict_list[self.start_idx:self.end_idx]
    ...
    def __getitem__(self, idx):
        ...
        path = train_dict['image_path']
        path_token = path.split('\\')
        image_name = path_token[-1]
        ...
        keypoints_2d =  np.array(train_dict['keypoints2d'])
        joints3d = np.array(train_dict['xyz_anno'])
        _K = np.array(train_dict['cam_anno'])
...
class Hanco_with_Mediapipe_JIttered(Dataset):
    def __init__(self, config, set_type="train", limit=2e5, device='cpu'):
        self.device = device
        self.image_dir = os.path.join(config.HANCO_DATA_ROOT, "rgb")
        self.img_size = config.IMG_SIZE
    ...
    def __getitem__(self, idx):
        ...
        # Scale jittering
        scale_jittering = scale_jittering * np.random.randn() + 0.5
        ...
        # joint augmentation
        tj = joint_transform(keypoints_2d[:, :2] * 256, center, affinetrans) / 256
        keypoints_2d[:, :2] = tj

        bb, crop_joint= crop_bb(keypoints_2d, [self.img_size, self.img_size], margin=10)
        tensor_img = pil_to_tensor(img)
        cropped_img = to_pil_image(tensor_img[:, bb[0, 1]:bb[1, 1], bb[0, 0]:bb[1, 0]])
...
if __name__=='__main__':
    ...
    dataset = Hanco_with_Mediapipe_JIttered(cfg, set_type='eval', device=device)
    print(len(dataset))
    # import pdb; pdb.set_trace()
    for i in range(0, len(dataset), len(dataset)//50):
        ...
        img = draw_joint(data['image'], data['keypoints3DD'], data['K'])
