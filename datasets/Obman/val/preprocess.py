...
files = os.listdir(os.path.join('val','rgb_hand'))
...
with mp_hands.Hands(
    ...
    for step, file_name in enumerate(files):
        file_name = file_name
        image = cv2.imread(os.path.join('val','rgb_hand', file_name), cv2.COLOR_BGR2RGB)
    ...
    with open(os.path.join('val', 'mediapipe_pixel.json'), 'w') as outfile:
