...
# matches run_joint_ablation.sh: warm-start rgbd_real_0613, lr 1e-5, 25 ep, best.pt
# selected on the held-out session, all re-scored by eval_iphone.py.
set -e
...
python train_mobrecon_rgbd.py --exp ft_fair_s --datasets iphone $COMMON

echo "======== [2/3] replay-S (iphone x8 + DexYCB 4k) ========"
python train_mobrecon_rgbd.py --exp replay_s --datasets iphone,dexycb \
    --iphone_repeat 8 --source_limit 4000 --dexycb_root "$DEXYCB" $COMMON

echo "======== [3/3] joint-S (iphone x8 + DexYCB 15k + HO3D 15k) ========"
python train_mobrecon_rgbd.py --exp joint_s --datasets iphone,dexycb,ho3d \
