...
ZED counterpart of `scripts/infer/infer_rgbd_femtobolt.py`. The ZED is a *passive stereo* depth
...
`scripts/infer/infer_rgbd_femtobolt.py`; only the camera front-end is swapped for the ZED SDK.
...
Examples
--------
  python scripts/infer/infer_rgbd_zed.py --ckpt mobrecon_ckpt/rgbd_real_0613/best.pt --source zed
  python scripts/infer/infer_rgbd_zed.py --ckpt <ckpt> --source svo --svo rec.svo2 --no-gui --save out/
  python scripts/infer/infer_rgbd_zed.py --ckpt <ckpt> --source selftest        # no hardware
"""
