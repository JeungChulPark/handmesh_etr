...
model + per-frame path as scripts/tools/unity_stream_hand.py, with the iPad as the sensor
...
The inferred joints are streamed out as the scripts/tools/unity_stream_hand.py UDP JSON packet.
...
Examples
--------
  python scripts/infer/infer_ipad_stream.py                        # GUI + joints/video -> local Unity
  python scripts/infer/infer_ipad_stream.py --udp none --udp-video none   # OpenCV overlay only
  python scripts/infer/infer_ipad_stream.py --udp-back 9751        # + reply joints to the iPad too
  python scripts/infer/infer_ipad_stream.py --selftest             # loopback wiring test, no iPad
"""
