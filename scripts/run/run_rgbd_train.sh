...
# Pipeline validated end-to-end by `python scripts/tools/smoke_rgbd.py`.
#
#   bash scripts/run/run_rgbd_train.sh                 # default: real RGB-D run
#   bash scripts/run/run_rgbd_train.sh warmup          # ToF depth-branch warm-up (MSRA only)
...
case "$MODE" in
  main)
    ...
    exec python scripts/train/train_mobrecon_rgbd.py \
  ...
  all)
    ...
    ;;
  *)
    echo "usage: bash scripts/run/run_rgbd_train.sh [main|warmup|all]"; exit 1 ;;
esac
