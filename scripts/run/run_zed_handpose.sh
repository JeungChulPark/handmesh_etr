...
#   bash scripts/run/run_zed_handpose.sh                 # live GUI window
#   CKPT=mobrecon_ckpt/rgbd_lidar_0613_ft5e6/best.pt bash scripts/run/run_zed_handpose.sh
#   bash scripts/run/run_zed_handpose.sh --no-gui --save out/   # headless, dump overlays
#
...
exec python scripts/infer/infer_rgbd_zed.py \
  --ckpt "$CKPT" \
  --source zed \
