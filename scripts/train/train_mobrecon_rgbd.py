...
This mirrors `scripts/train/train_mobrecon_jittering_3d.py` but trains the early-fusion 4-channel
...
Run:
    ...
    python scripts/train/train_mobrecon_rgbd.py --exp rgbd_v0 --epoch 30 --depth_source cache \
        --pretrain pretrain/100.pt
    # train on real depth, HanCo + DexYCB + HO3D + MSRA(ToF) concatenated:
    python scripts/train/train_mobrecon_rgbd.py --exp rgbd_real --datasets hanco,dexycb,ho3d,msra \
    ...
    python scripts/train/train_mobrecon_rgbd.py --exp rgb_only --cfg ./configs/configs_rgbd.yaml --rgb_only
...
def build_dataset(args, depth_cfg):
    ...
    for n in names:
        if n == "hanco":
            ...
            # Real iPhone LiDAR captures with pseudo-3D GT (scripts/capture/make_iphone_gt.py).
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cfg", default="./configs/configs_rgbd.yaml")
    ...
    parser.add_argument("--iphone_root", default="rgbd_captures", help="iPhone captures dir (cap_*_gt.json from scripts/capture/make_iphone_gt.py)")
