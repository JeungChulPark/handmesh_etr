...
`scripts/infer/infer_rgbd_zed.py`: confirm the hand is in range, depth coverage on the hand,
and capture frames/clips for offline replay. Reuses `ZEDSource` from
scripts/infer/infer_rgbd_zed.py, so capture/intrinsics behaviour is identical to inference.
...
Examples
--------
  python scripts/capture/view_zed.py                              # live, GUI
  python scripts/capture/view_zed.py --source svo --svo rec.svo2  # replay a recording
  python scripts/capture/view_zed.py --save captures              # enable 's' snapshots / 'r' record
  python scripts/capture/view_zed.py --no-gui --save captures --max_frames 300   # headless dump
"""
