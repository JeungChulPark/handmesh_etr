...
# Every best.pt is then re-scored by eval_iphone.py so all numbers share one metric.
set -e
...
python train_mobrecon_rgbd.py --exp ft_fair --datasets iphone $COMMON

echo "======== [2/3] replay (iphone x8 + DexYCB 4k) ========"
python train_mobrecon_rgbd.py --exp replay_iphone --datasets iphone,dexycb \
    --iphone_repeat 8 --source_limit 4000 --dexycb_root "$DEXYCB" $COMMON

echo "======== [3/3] joint (iphone x8 + DexYCB 15k + HO3D 15k) ========"
python train_mobrecon_rgbd.py --exp joint_iphone --datasets iphone,dexycb,ho3d \
...
for exp in ft_fair replay_iphone joint_iphone; do
    python eval_iphone.py --ckpt "mobrecon_ckpt/$exp/best.pt" 2>&1 | grep "eval_iphone]"
done
python eval_iphone.py --ckpt mobrecon_ckpt/ef_iphone_v2/best.pt 2>&1 | grep "eval_iphone]"
python eval_iphone.py --ckpt "$PRETRAIN" 2>&1 | grep "eval_iphone]"
echo "======== DONE ========"
