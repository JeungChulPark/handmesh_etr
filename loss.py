...
def l2_loss(pred, gt, is_valid=None, drop_nan=False):
    """L1 loss
...
def myLoss3d(pred_dict, gt_dict):
    loss_dict = dict()

    # Since mobrecon estimates verts -> change to joint

    # joint3d_loss = l1_loss(pred_dict['verts'], gt_dict['keypoints3D'])
    # loss_dict['joint3d_loss'] = joint3d_loss.item()
    joint2d_loss = l1_loss(pred_dict['joint_img'], gt_dict['keypoints2D'])
...
def myLoss( pred_dict, gt_dict):
    loss_dict = dict()

    joint3d_loss = l1_loss(pred_dict['keypoints3D'], gt_dict['keypoints3D'])
    loss_dict['joint3d_loss'] = joint3d_loss.item()
    # joint2d_loss = l1_loss(pred_dict['keypoints'], gt_dict['keypoints'])
    # loss_dict['joint2d_loss'] = joint2d_loss.item()
    loss_dict['total'] = joint3d_loss # + joint2d_loss

    return loss_dict

def myLoss25d(pred_dict, gt_dict):    
    loss_dict = dict()

    
    joint2d_loss = l2_loss(pred_dict['keypoints'].float()[:, :, :2], gt_dict['keypoints'].float()[:, :, :2])
    joint3d_loss = l1_loss(pred_dict['keypoints3D'], gt_dict['keypoints'].float())
    # joint2d_loss = l1_loss(pred_dict['keypoints'], gt_dict['keypoints'])
    loss_dict['joint2d_loss'] = joint2d_loss.item()
    loss_dict['total'] = joint2d_loss  + joint3d_loss
