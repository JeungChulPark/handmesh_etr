...
`hybrid_B` is one of two hybrid lifters trained by `scripts/run/run_BA.sh` (both warm-started
from `rgbd_real_0613`, datasets `dexycb,ho3d,iphone`, 40 epochs). The model class
is `HybridLifter` in `scripts/train/train_hybrid.py`; the two variants differ only in `--mode`:
...
## 3. Live ZED inference — `scripts/infer/infer_zed_hybrid.py`
...
* per-frame inference: same path as `scripts/infer/infer_hybrid.py` — MediaPipe uv →
...
```bash
DISPLAY=:1 python scripts/infer/infer_zed_hybrid.py --source zed            # live GUI, NEURAL depth
DISPLAY=:1 python scripts/infer/infer_zed_hybrid.py --source zed --zed_depth_mode PERFORMANCE
python scripts/infer/infer_zed_hybrid.py --no-gui --save zed_hybrid_out/    # headless: PNGs + metrics.jsonl
python scripts/infer/infer_zed_hybrid.py --source svo --svo rec.svo2        # replay a recording
python scripts/infer/infer_zed_hybrid.py --source selftest                 # no hardware, wiring check
```
...
| Path | Role |
|---|---|
| `scripts/infer/infer_zed_hybrid.py` | **new** — live ZED / SVO inference for hybrid_B (locked). |
| `scripts/train/train_hybrid.py` | `HybridLifter`, both A/B modes; training entrypoint. |
| `scripts/run/run_BA.sh` | trains B then A. |
| `scripts/infer/infer_hybrid.py` | offline hybrid inference on `cap_*.json` captures. |
| `scripts/infer/infer_rgbd_zed.py` | `ZEDSource` (camera front-end, reused). |
